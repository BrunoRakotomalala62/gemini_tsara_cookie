#!/usr/bin/env python3
"""
Gemini API Proxy v5 — Chat + Image Recognition + Image Editing
=====================================================================
...
CONFIG = {"port":int(os.getenv("PORT","8080")),"host":"0.0.0.0","cookie_file":"cookies.txt","gemini_bl":os.getenv("GEMINI_BL","boq_assistant-bard-web-server_20260716.08_p0"),"timeout":90,"xsrf_refresh":3600,"pool_connections":10,"pool_maxsize":20}
...
def load_cookies(cookie_file=None):
    ...
    if b64:
        try: raw = base64.b64decode(b64).decode("utf-8"); print(f"✓ Cookies chargés depuis GEMINI_COOKIES_B64 ({len(b64)} chars)")
        except Exception as e: print(f"⚠ GEMINI_COOKIES_B64 invalide: {e}")
    if not raw and cookie_file:
        if os.path.exists(cookie_file):
            with open(cookie_file) as f: raw = f.read()
            print(f"✓ Cookies chargés depuis {cookie_file}")
        else: print(f"⚠ Fichier introuvable: {cookie_file}")
    if not raw: print("💥 Aucun cookie ! Mets GEMINI_COOKIES_B64=<base64>"); sys.exit(1)
    cookies = {}
    ...
    if not cookies: print("💥 Aucun cookie parsé !"); sys.exit(1)
    return cookies
...
def extract_xsrf_from_page():
    ...
    headers = {"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36","Accept":"text/html,application/xhtml+xml","Cookie":cookie_header(_cookies)}
    try:
        if HAS_HTTPX and _http_client: r = _http_client.get(url, headers=headers, timeout=15, follow_redirects=True); raw = r.text
        else:
    ...
    except Exception as e: print(f"[XSRF] Échec extraction: {e}"); return None
    m = re.search(r'"SNlM0e"\s*:\s*"([^"]+)"', raw)
    if m:
        print(f"[XSRF] Token extrait: {m.group(1)[:30]}...")
        pm = re.search(r'"KnDnFf"\s*:\s*"([^"]+)"', raw)
        if pm: _push_id = pm.group(1); print(f"[XSRF] Push-ID extrait: {_push_id}")
        return m.group(1)
    print("[XSRF] Token non trouvé"); return None
    ...
    if not _xsrf_token or (now - _xsrf_ts) > CONFIG["xsrf_refresh"]:
        with _cache_lock:
            if not _xsrf_token or (now - _xsrf_ts) > CONFIG["xsrf_refresh"]:
                ...
                if token: _xsrf_token = token; _xsrf_ts = now
    return _xsrf_token
...
def call_gemini(prompt, model="flash", image_url=None):
    ...
    if image_url:
        try: file_ref, filename = upload_image_to_gemini(image_url)
        except Exception as e: raise RuntimeError(f"Image upload failed: {e}")
    inner = [None] * 80
    ...
    try:
        if HAS_HTTPX and _http_client: r = _http_client.post(url, content=body, headers=headers, timeout=CONFIG["timeout"]); raw = r.text
        else:
            import urllib.request as ur
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/gemini":
            ...
            try:
                ...
                result = {"success":True,"uid":uid,"prompt":prompt,"model":model,"response":response,"elapsed_ms":elapsed}
            ...
            self.send_json({"status":"ok","models":list(MODELS.keys()),"xsrf_cached":bool(_xsrf_token),"xsrf_age_s":int(time.time()-_xsrf_ts) if _xsrf_token else None,"cookies_loaded":len(_cookies),"http_backend":"httpx" if HAS_HTTPX else "urllib","gemini_bl":CONFIG["gemini_bl"],"cookie_source":"env:GEMINI_COOKIES_B64" if os.getenv("GEMINI_COOKIES_B64") else "file","push_id":_push_id})
        elif parsed.path == "/":
            self.send_json({"service":"Gemini API Proxy v5","features":["Chat","Image Recognition","Image Editing","Auto-XSRF","Env vars"],"usage":"GET /api/gemini?prompt=...&image_url=...&uid=ID&model=MODELE","models":list(MODELS.keys())})
        else: self.send_json({"error":"Not found"}, 404)

def main():
    global _cookies, _http_client
    import argparse
    parser = argparse.ArgumentParser(description="Gemini API Proxy v5")
    ...
    print(f"  SID: {'✓' if 'SID' in _cookies else '✗'}  1PSID: {'✓' if '__Secure-1PSID' in _cookies else '✗'}  COMPASS: {'✓' if 'COMPASS' in _cookies else '✗'}  SAPISID: {'✓' if 'SAPISID' in _cookies else '✗'}")
    if HAS_HTTPX:
    ...
    print(f"\n🚀 Gemini API Proxy v5 → http://0.0.0.0:{CONFIG['port']}")
    print(f"   GET /api/gemini?prompt=...&image_url=...&uid=123")
