#!/usr/bin/env python3
"""
Gemini API Proxy v3 — Déploiement-ready avec variables d'environnement
=====================================================================
...
Endpoints:
  ...
  GET /api/health
"""
import json, uuid, hashlib, ssl, time, re, os, sys, threading, base64
...
# ═══════════════════════════════════════════════════════════════════════════════
# LOAD COOKIES — priorité : env var > fichier
# ═══════════════════════════════════════════════════════════════════════════════

def load_cookies(cookie_file=None):
    """
    Charge les cookies. Priorité :
    1. GEMINI_COOKIES_B64 (env var) — base64 du fichier Netscape
    2. Fichier cookie_file passé en argument
    """
    raw = None
    ...
    if not raw:
        ...
        print("   → Mets GEMINI_COOKIES_B64=<base64> dans les env vars")
        print("   → Ou passe un fichier: python3 gemini-api.py 8080 cookies.txt")
        sys.exit(1)

    # Parser le format Netscape
    cookies = {}
    ...
    if not cookies:
        print("💥 Aucun cookie parsé ! Vérifie le format Netscape.")
        sys.exit(1)
...
def call_gemini(prompt, model="flash"):
    ...
    if texts:
        return RE_CODE_CLEAN.sub('', texts[-1].strip())
    return ""
...
class GeminiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/gemini":
            ...
            self.send_json({
                "service": "Gemini API Proxy v3",
                "features": ["Auto-XSRF", "Keep-Alive HTTP/2", "Env var support"],
                "usage": "GET /api/gemini?prompt=TEXTE&uid=ID&model=MODELE",
                "models": list(MODELS.keys()),
            })
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Gemini API Proxy v3")
...
║                                                        ║
║  GET /api/gemini?prompt=bonjour&uid=123&model=flash     ║
║  GET /api/health                                        ║
║                                                        ║
║  Env vars ✅  |  Auto-XSRF ✅  |  Keep-Alive ✅         ║
╚══════════════════════════════════════════════════════════╝
""")
